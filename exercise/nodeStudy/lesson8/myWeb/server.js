...
var fs = require("fs");
// 载入query模块
var myQuery = require("./utils/myQuery.js");
const HOST = "localhost",PORT = 80;
var server = http.createServer(function(req,res){
	var url = req.url.split("?")[0];
	var params = req.url.split("?")[1];
	if(url != "/favicon.ico"){//判断是否使用了浏览器标题默认图片
		if(url == "/"){
			...
			req.on("data",function(data){
				...
				// res.write("your message was submited\n username"+params.username+"\t sex"+params.sex+"\t comment"+params.comment);
				// 插入数据到数据库
				myQuery.query("INSERT user_comment (username,password,sex,comment,is_delete)VALUES(?,?,?,?,?)",params,function(result){
					console.log(result);
				});
				res.write("success");//返回成功标示
				res.end();//res.end()与res.write成双出现的
			});
		}else if(url == "/userCommentController.do"){
			// 更改数据库数据
			myQuery.query('UPDATE user_comment SET username = "youyouhei" WHERE id =14',{},function(result){
				// 查询数据库的数据
				myQuery.query("SELECT * FROM user_comment",{},function(result){
					// console.log(result);//json数组
			...
			res.writeHead(200,{"Content-Type":"text/html"});
			fs.createReadStream(path.join(__dirname,"view",url+".html")).pipe(res);
		}else if(url == "/js/react.js"){
			res.writeHead(200,{"Content-Type":"text/javascript"});
			fs.createReadStream(path.join(__dirname,"public",url)).pipe(res);
		}else if(url == "/js/react-dom.js"){
			res.writeHead(200,{"Content-Type":"text/javascript"});
			fs.createReadStream(path.join(__dirname,"public",url)).pipe(res);
		}else if(url == "/js/browser.js"){
			res.writeHead(200,{"Content-Type":"text/javascript"});
			fs.createReadStream(path.join(__dirname,"public",url)).pipe(res);
		}else {
			return;
